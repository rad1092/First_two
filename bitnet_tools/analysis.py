...
class AnalysisError(ValueError):
    """Raised when CSV content cannot be analyzed safely."""
...
def _percentile(values: list[float], p: float) -> float:
    ...
    if not ordered:
        raise AnalysisError("values cannot be empty")
    idx = (len(ordered) - 1) * p
    lower = int(idx)
...
def _parse_csv_text(csv_text: str) -> tuple[list[str], list[dict[str, str]]]:
    if not csv_text.strip():
        raise AnalysisError("CSV content is empty")
...
def _read_text_with_fallbacks(path: Path) -> str:
    ...
    for enc in ("utf-8-sig", "utf-8", "cp949"):
        try:
            ...
            continue
    raise AnalysisError("Unsupported file encoding. Use UTF-8 or CP949.")
...
def summarize_rows(rows: list[dict[str, str]], columns: list[str]) -> DataSummary:
    ...
    for col in columns:
        ...
        if values and len(values) == len(non_missing):
            ...
            low, high = stats["q1"] - 1.5 * iqr, stats["q3"] + 1.5 * iqr
            stats["outlier_count"] = float(sum(1 for v in values if v < low or v > high))
